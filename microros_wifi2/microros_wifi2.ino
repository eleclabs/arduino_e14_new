...
IPAddress local_IP(10, 131, 127, 200); // Set your Static IP address
IPAddress gateway(10, 131, 127, 1);  // Set your Gateway IP address
...
// Encoder Pins
#define LeftEncoder_C1 36
#define LeftEncoder_C2 39
#define RightEncoder_C1 34
#define RightEncoder_C2 35

// Pin Definitions
#define IN1_PIN 25    //left forward
#define IN2_PIN 26    //left backward
#define IN3_PIN 32    //right forward
#define IN4_PIN 33    //right backward
...
void servo_callback(const void* msgin) 
{
  ...
  int servo_position;
  servo_position = limitToMaxValue(angle, 90);
...
void setMotorSpeed(int speedLeft, int speedRight) 
{
  if (speedLeft > 0) {
    ...
    analogWrite(IN1_PIN, abs(limitToMaxValue(speedLeft, 200)));
  } else {
    digitalWrite(IN1_PIN, LOW);
    analogWrite(IN2_PIN, abs(limitToMaxValue(speedLeft, 200)));
  }

  // Set right motor direction and speed
  if (speedRight > 0) {
    digitalWrite(IN3_PIN, LOW);
    analogWrite(IN4_PIN, abs(limitToMaxValue(speedRight, 200)));
  } else {
    digitalWrite(IN4_PIN, LOW);
    analogWrite(IN3_PIN, abs(limitToMaxValue(speedRight, 200)));
  }
  ...
  if (speedLeft == 0 && speedRight == 0  ) {
    ...
    digitalWrite(IN1_PIN, LOW);
    analogWrite(IN1_PIN, abs(limitToMaxValue(speedLeft, 200)));
    analogWrite(IN4_PIN, abs(limitToMaxValue(speedLeft, 200)));
  }
}
...
// Configures static IP address
  ...
  set_microros_wifi_transports("A56", "42117622", "10.131.127.212", 8888);  //IP ของเครื่อง Host PC
  ...
  // create node
  RCCHECK(rclc_node_init_default(&node, "esp3_micro_ros", "", &support));
