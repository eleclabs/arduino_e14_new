...
void handleRoot() {
  ...
  <head>
    ...
    <script>
      ...
      function startMove(action) {
        stopMove(); // หยุดคำสั่งก่อนหน้าที่อาจทำงานอยู่
        interval = setInterval(() => fetch(`/${action}`), 100); // ส่งคำขอ HTTP ทุก 100ms
      }

      function stopMove() {
        if (interval) {
          clearInterval(interval); // ยกเลิกการส่งคำขอ
          interval = null;
          fetch('/stop'); // ส่งคำสั่งหยุด
        }
      }
  ...
  <body>
    ...
    <p><button class="button" onmousedown="startMove('forward')" onmouseup="stopMove()" ontouchstart="startMove('forward')" ontouchend="stopMove()">FORWARD</button></p>
    <div style="clear: both;">
      <p>
        ...
        <button class="button button2" onclick="stopMove()">STOP</button>
    ...
    <p><button class="button" onmousedown="startMove('reverse')" onmouseup="stopMove()" ontouchstart="startMove('reverse')" ontouchend="stopMove()">REVERSE</button></p>
