...
const char* AGENT_IP      = "192.168.1.128";   // ROS 2 PC/RPi IP
...
// Motor control pins (H-bridge / driver)
int INA1 = 26; 
int INB1 = 25;
int INA2 = 32;  
int INB2 = 33;
...
// Motor control: left + right speeds in [-100, 100]
...
void cmd_vel_callback(const void *msgin) {
  ...
  // Flip sign so +linear.x is physical forward
  leftspeed  = -leftspeed;
...
void setup() {
  ...
  // ---- micro-ROS WiFi transport (handles WiFi internally) ----
  ...
  // Optional debug after a short delay
  delay(3000);
  ...
  Serial.println("Robot initialized: WiFi micro-ROS, /cmd_vel subscriber, /odom + /tf publisher");
}
